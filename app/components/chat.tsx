    ...
    <div className="fixed bottom-0 right-0 mb-4 mr-4 w-96 bg-white rounded-lg shadow-lg flex flex-col ">
      <div className="flex items-center justify-between p-3 border-b border-gray-200">
        <div className="flex items-center">
          <img className="w-10 h-10 object-cover rounded-full mr-3" src={userPfp ? imgurl + userPfp : imgurl + `defaultimg.svg`}  alt={selectedUser} />
      ...
      <div className="h-72 overflow-y-auto p-3" ref={messagesContainerRef}>
        <ul>
          {messages.map((message) => (
            <li key={message._id} className={`${message.userid === userid ? 'text-right' : 'text-left'} mb-2`}>
